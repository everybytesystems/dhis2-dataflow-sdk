...
class DHIS2Module(
    ...
    suspend fun initialize(): NetworkResult<String> {
        return try {
            ...
            NetworkResult.Error(e, "Failed to initialize DHIS2 module")
        }
    }
...
class DHIS2Client(
    ...
    suspend fun testConnection(): NetworkResult<String> {
        return try {
            val response = httpClient.get("$baseUrl/system/info") {
                headers {
            ...
            NetworkResult.Error(e, "Connection test failed")
        }
    }
    ...
    suspend fun get(endpoint: String): NetworkResult<String> {
        return try {
            ...
            NetworkResult.Error(e, "GET request failed")
        }
    }
    ...
    suspend fun post(endpoint: String, data: String): NetworkResult<String> {
        return try {
            ...
            NetworkResult.Error(e, "POST request failed")
        }
    }
    ...
    suspend fun put(endpoint: String, data: String): NetworkResult<String> {
        return try {
            ...
            NetworkResult.Error(e, "PUT request failed")
        }
    }
    ...
    suspend fun delete(endpoint: String): NetworkResult<String> {
        return try {
            ...
            NetworkResult.Error(e, "DELETE request failed")
        }
    }
    
    private fun encodeCredentials(): String {
        val credentials = "${config.username}:${config.password}"
        return credentials.encodeToByteArray().let { 
            // Simple base64 encoding - in real implementation use proper base64
            it.toString()
        }
    }
}
