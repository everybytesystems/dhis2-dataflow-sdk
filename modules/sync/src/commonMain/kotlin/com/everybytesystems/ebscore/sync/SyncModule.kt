...
sealed class SyncStatus {
    ...
    data class Failed(val error: Throwable, val retryCount: Int) : SyncStatus()
...
class EBSCoreSyncEngine(
    ...
    override suspend fun startSync(): Flow<SyncStatus> = flow {
        ...
        try {
            ...
            val duration = System.currentTimeMillis() // Simplified
            currentStatus = SyncStatus.Completed(syncedRecords, duration)
            emit(currentStatus)
            
        } catch (e: Exception) {
            currentStatus = SyncStatus.Failed(e, 0)
            emit(currentStatus)
        }
    }
